...
<div class="min-h-screen p-8">
    <div class="glass-effect rounded-2xl shadow-xl p-8 mb-8 animate-fadeInUp">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-4xl font-bold gradient-text">Subscription Requests</h1>
                <p class="text-gray-600 mt-2">Manage upgrade requests and approvals</p>
            </div>
    ...
    <div class="glass-effect rounded-2xl shadow-xl overflow-hidden animate-fadeInUp" style="animation-delay: 0.2s">
        <table class="min-w-full">
            <thead class="gradient-bg">
                <tr>
                    <th class="px-6 py-4 text-left text-xs font-medium text-white uppercase">User</th>
                    <th class="px-6 py-4 text-left text-xs font-medium text-white uppercase">Current Plan</th>
                    <th class="px-6 py-4 text-left text-xs font-medium text-white uppercase">Requested Plan</th>
                    <th class="px-6 py-4 text-left text-xs font-medium text-white uppercase">Phone</th>
                ...
                <tr class="hover:bg-white/70 transition-all">
                    ...
                    <td class="px-6 py-4">
                        @if($request->status === 'pending')
                            <span class="px-3 py-1 text-xs font-medium rounded-full bg-gradient-to-r from-yellow-500 to-orange-500 text-white">Pending</span>
                        @elseif($request->status === 'approved')
                            <span class="px-3 py-1 text-xs font-medium rounded-full bg-gradient-to-r from-green-500 to-emerald-500 text-white">Approved</span>
                        @else
                            <span class="px-3 py-1 text-xs font-medium rounded-full bg-gradient-to-r from-red-500 to-pink-500 text-white">Rejected</span>
                        @endif
                    </td>
                    ...
                    <td class="px-6 py-4">
                        @if($request->status === 'pending')
                            <form method="POST" action="{{ route('admin.subscription.approve', $request) }}" class="inline">
                                @csrf
                                <button type="submit" class="btn-premium px-5 py-2.5 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl font-semibold shadow-lg mr-2">✓ Approve</button>
                            </form>
                            <form method="POST" action="{{ route('admin.subscription.reject', $request) }}" class="inline">
                                @csrf
                                <button type="submit" class="btn-premium px-5 py-2.5 bg-gradient-to-r from-red-500 to-pink-600 text-white rounded-xl font-semibold shadow-lg">✕ Reject</button>
                            </form>
                        @else
