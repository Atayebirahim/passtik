...
<div class="min-h-screen p-8">
    <div class="glass-effect rounded-2xl shadow-xl p-8 mb-8 animate-fadeInUp">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-4xl font-bold gradient-text">All Vouchers</h1>
                <p class="text-gray-600 mt-2">System-wide voucher overview</p>
            </div>
    ...
    <div class="glass-effect rounded-2xl shadow-xl overflow-hidden animate-fadeInUp" style="animation-delay: 0.2s">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="gradient-bg">
                <tr>
                    <th class="px-6 py-4 text-left text-xs font-medium text-white uppercase">Code</th>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-medium text-white uppercase">Duration</th>
                    ...
                    <tr class="hover:bg-white/70 transition-all">
                        ...
                        <td class="px-6 py-4">
                            @if($voucher->status === 'active')
                                <span class="px-3 py-1 text-xs font-medium rounded-full bg-gradient-to-r from-green-500 to-emerald-500 text-white">Active</span>
                            @elseif($voucher->status === 'used')
                                <span class="px-3 py-1 text-xs font-medium rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 text-white">Used</span>
                            @elseif($voucher->status === 'expired')
                                <span class="px-3 py-1 text-xs font-medium rounded-full bg-gradient-to-r from-red-500 to-pink-500 text-white">Expired</span>
                            @else
                                <span class="px-3 py-1 text-xs font-medium rounded-full bg-gray-100 text-gray-800">{{ ucfirst($voucher->status) }}</span>
                            @endif
                        </td>
